...
int harga,t,sisa,k,l,nomor,bill1(order_id,mall,i,jam_pilih,t,harga), beli(order_id,mall,i,jam_pilih), penvil(order_id,mall), dmall(order_id,mall), detos(order_id,mall), margo(order_id,mall), order1(order_id), terimakasih1(order_id), mall,jam_pilih;
void ketjek();
...
void totalbill()   //*Modular Pembayaran*//
{
	FILE *fptr;                                                   //*variabel pointer*//
	printf("\nTotal Harga: Rp. %d\n", totalsemua);
	fptr = fopen("Gopay.txt","r");                         //*Membaca data dari file OrderHistory.txt*//
	fscanf(fptr,"%d", &saldo);
	printf("Saldo:%d",saldo);
	if(saldo!=totalsemua){
	printf("\nMasukkan Jumlah Uang Anda: Rp.");
    fptr = fopen("Gopay.txt","w");                         //*Memasukkan data ke file OrderHistory.txt*//
   ...
   if ((fptr = fopen("Gopay.txt","r")) == NULL){
...
int order(order_id,driver_id)   //*Modular Order Jarak*//
{
   ...
   char  f[30];
    ...
    struct data data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15;

   strcpy(data1.f, "Fakultas Teknik");
   strcpy(data2.f, "Vokasi");
   strcpy(data3.f, "Pusgiwa");
   strcpy(data4.f, "Fakultas MIPA");
   strcpy(data5.f, "Fakultas Ilmu Keperawatan");
   strcpy(data6.f, "Fakultas Kesehatan Masyarakat");
   strcpy(data7.f, "RIK");
   strcpy(data8.f, "Balairung");
   strcpy(data9.f, "Fakultas Hukum");
   strcpy(data10.f, "FakultasPsikologi");
   strcpy(data11.f, "Fakultas Ilmu Budaya");
   strcpy(data12.f, "Fakultas Ekonomi");
   strcpy(data13.f, "Tempat Penjemputan");
   strcpy(data14.f, "Tempat Tujuan");
   strcpy(data15.f, "Zona");
    ...
    if(i==0){printf("\t\t\t|%-30s    \t%-5s%d     |%-35s|\n", data13.f,data15.f,arr[i][0],data1.f);}
    else if(i==1){printf("\t\t\t|%-30s    \t%-5s%d     |%-35s|\n", data13.f,data15.f,arr[i][0],data2.f);}
    else if(i==2){printf("\t\t\t|%-30s    \t%-5s%d     |%-35s|\n", data13.f,data15.f,arr[i][0],data3.f);}
    else if(i==3){printf("\t\t\t|%-30s    \t%-5s%d     |%-35s|\n", data13.f,data15.f,arr[i][0],data4.f);}
    else if(i==4){printf("\t\t\t|%-30s    \t%-5s%d     |%-35s|\n", data13.f,data15.f,arr[i][0],data5.f);}
    else if(i==5){printf("\t\t\t|%-30s    \t%-5s%d     |%-35s|\n", data13.f,data15.f,arr[i][0],data6.f);}
    else if(i==6){printf("\t\t\t|%-30s    \t%-5s%d     |%-35s|\n", data13.f,data15.f,arr[i][0],data7.f);}
    else if(i==7){printf("\t\t\t|%-30s    \t%-5s%d     |%-35s|\n", data13.f,data15.f,arr[i][0],data8.f);}
    else if(i==8){printf("\t\t\t|%-30s    \t%-5s%d     |%-35s|\n", data13.f,data15.f,arr[i][0],data9.f);}
    else if(i==9){printf("\t\t\t|%-30s    \t%-5s%d     |%-35s|\n", data13.f,data15.f,arr[i][0],data10.f);}
    else if(i==10){printf("\t\t\t|%-30s    \t%-5s%d     |%-35s|\n", data13.f,data15.f,arr[i][0],data11.f);}
    else if(i==11){printf("\t\t\t|%-30s    \t%-5s%d     |%-35s|\n", data13.f,data15.f,arr[i][0],data12.f);}
    if(j==0){printf("\t\t\t|%-30s    \t%-5s%d     |%-35s|\n", data14.f, data15.f,arr[j][0],data1.f);}
    else if(j==1){printf("\t\t\t|%-30s    \t%-5s%d     |%-35s|\n", data14.f, data15.f,arr[j][0],data2.f);}
    else if(j==2){printf("\t\t\t|%-30s    \t%-5s%d     |%-35s|\n", data14.f, data15.f,arr[j][0],data3.f);}
    else if(j==3){printf("\t\t\t|%-30s    \t%-5s%d     |%-35s|\n", data14.f, data15.f,arr[j][0],data4.f);}
    else if(j==4){printf("\t\t\t|%-30s    \t%-5s%d     |%-35s|\n", data14.f, data15.f,arr[j][0],data5.f);}
    else if(j==5){printf("\t\t\t|%-30s    \t%-5s%d     |%-35s|\n", data14.f, data15.f,arr[j][0],data6.f);}
    else if(j==6){printf("\t\t\t|%-30s    \t%-5s%d     |%-35s|\n", data14.f, data15.f,arr[j][0],data7.f);}
    else if(j==7){printf("\t\t\t|%-30s    \t%-5s%d     |%-35s|\n", data14.f, data15.f,arr[j][0],data8.f);}
    else if(j==8){printf("\t\t\t|%-30s    \t%-5s%d     |%-35s|\n", data14.f, data15.f,arr[j][0],data9.f);}
    else if(j==9){printf("\t\t\t|%-30s    \t%-5s%d     |%-35s|\n", data14.f, data15.f,arr[j][0],data10.f);}
    else if(j==10){printf("\t\t\t|%-30s    \t%-5s%d     |%-35s|\n", data14.f, data15.f,arr[j][0],data11.f);}
    else if(j==11){printf("\t\t\t|%-30s    \t%-5s%d     |%-35s|\n", data14.f, data15.f,arr[j][0],data12.f);}
    printf("\t\t\t----------------------------------------------------------------------------------------\n");
    ...
    printf("\nM E N U : \n1. Order KUY \n2. Menu Sebelumnya ");
    		...
    		switch(b)
    		{
    			...
    			case 2:
    				...
    				break;
    			default:
            	printf("Maaf, fitur yang anda pilih tidak ada didalam menu..!! Silahkan Coba lagi :)\n");   //*Keterangan Error*//
           		system("pause");
           		getchar();                                                                                 //*Error Handling Huruf*//
           		system("cls");
				kuyjek();
...
int bill1(order_id,mall,i,jam_pilih,t,harga)
{
   ...
   char  tkt[30];
    ...
    struct ket ket1, ket2, ket3, ket4, ket5, ket6, ket7, ket8, ket9, ket10, ket11, ket12, ket13, ket14, ket15;

   strcpy(ket1.tkt, "MARGO CITY XXI");
   strcpy(ket2.tkt, "CINEMAXX DETOS");
   strcpy(ket3.tkt, "CGV DEPOK MALL");
   strcpy(ket4.tkt, "PENVILL XXI");
   strcpy(ket5.tkt, "VENOM");
   strcpy(ket6.tkt, "SMALLFOOT");
   strcpy(ket7.tkt, "ARUNA & LIDAHNYA");
   strcpy(ket8.tkt, "JHONNY ENGLISH 2");
   strcpy(ket9.tkt, "MUNAFIK 2");
   strcpy(ket10.tkt, "CRAZY RICH ASIANS");
   strcpy(ket11.tkt, "TEMPAT");
   strcpy(ket12.tkt, "FILM");
    system("cls");
...
{
    printf("\t\t\t======================================================================\n");
    printf("\t\t\t|\t\t\tANDA MENDAPATKAN TEMPAT DUDUK\t\t     |\n");
    printf("\t\t\t======================================================================\n");
    for(p=1;p<t+1;p++)
    {
        Enqueue(p);
